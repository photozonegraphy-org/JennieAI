...
async function loadTool(toolId){
  if(_loadedTools[toolId])return; // already executed

  var manifest=await fetchManifest();
  var url=manifest.tools[toolId];
  if(!url)throw new Error('This tool is not available yet.');

  // Fetch as text — avoids CORS preflight issues with script injection
  var res=await fetch(url,{mode:'cors',cache:'force-cache'});
  if(!res.ok)throw new Error('Could not load analysis module ('+res.status+'). Check CDN CORS headers.');
